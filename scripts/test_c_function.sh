...
test_function()
{
	...
	cc ./$DAY_PATH/${FULL_TEST_NAME}.c libs/Moumounator.a -Wl,--wrap,malloc -Wl,--wrap,free -Ilibs -lm -lbsd -o $OUTPUT_PATH/answer.out
	if [ $? -ne 0 ]; then
		continue
	fi
	# Compile test with user library
	cc -Wall -Wextra -Werror -fsanitize=address -g ./$DAY_PATH/${FULL_TEST_NAME}.c libs/libftprintf.a -Ilibs -Wl,--wrap,malloc -Wl,--wrap,free -o $OUTPUT_PATH/user.out 2> $OUTPUT_PATH/user.compilation_error.out
	if [ $? -ne 0 ]; then
